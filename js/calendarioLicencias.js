...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document
    .getElementById("btnCargarLicencia")
  ...
  btnCrearLicenciaPara.addEventListener("click", async () => {
    const modalActual = bootstrap.Modal.getInstance(
      document.getElementById("modalCrearLicencia"),
    );
    modalActual.hide();

    const modalNuevo = new bootstrap.Modal(
      document.getElementById("modalCrearLicenciaOtro"),
    );
    modalNuevo.show();
  ...
  // =========================
  document
    .getElementById("btnCargarLicenciaOtro")
    .addEventListener("click", async () => {
  ...
  document
    .getElementById("btnVerMisLicencias")
    .addEventListener("click", async () => {
      try {
...
`;
        ...
        document.querySelectorAll(".btn-editar").forEach((btn) => {
          btn.addEventListener("click", (e) => {
            ...
            document.getElementById("fechaDesde").value =
              licencia.Fecha_Desde.split("T")[0];
            document.getElementById("fechaHasta").value =
              licencia.Fecha_Hasta.split("T")[0];
  ...
  async function cargarSubgrupos(grupo) {
    try {
      const response = await fetch(
        `${basePath}/api/licencias/subgrupos?grupo=${grupo}`,
      );
      const result = await response.json();
  ...
  async function renderCalendario() {
    ...
    if (modoTodos) {
      ...
      estructura = Object.keys(gruposMap)
        ...
        .map((grupoNombre) => ({
          tipo: "grupo",
          nombre: grupoNombre,
          usuarios: Object.values(gruposMap[grupoNombre]).sort((a, b) =>
      ...
      estructura = Object.keys(subgruposMap)
        ...
        .map((sub) => ({
          tipo: "subgrupo",
          nombre: sub,
          usuarios: Object.values(subgruposMap[sub]).sort((a, b) =>
            a.nombre.localeCompare(b.nombre),
          ),
        }));
    }
    ...
    if (
      estructura.length === 0 ||
      estructura.every((b) => b.usuarios.length === 0)
    ) {
      ...
      bloque.usuarios.forEach((usuario) => {
        ...
        dias.forEach((dia) => {
          ...
          const licenciaDelDia = usuario.licencias.find((l) => {
            ...
            const desdeTime = new Date(
              desde.getFullYear(),
              desde.getMonth(),
              desde.getDate(),
            ).getTime();
            const hastaTime = new Date(
              hasta.getFullYear(),
              hasta.getMonth(),
              hasta.getDate(),
            ).getTime();
          ...
          if (licenciaDelDia) {
            ...
            const estadoLicencia = String(licenciaDelDia.Estado || "")
              .trim()
              .toUpperCase();
            ...
            if (estadoLicencia === "PENDING") {
              ...
              switch (licenciaDelDia.TipoLic) {
                case "VACACIONES":
                  letra = "V";
                  claseExtra += " tipo-vacaciones";
                  break;
                case "COMPENSACIÓN DIA":
                  letra = "CD";
                  claseExtra += " tipo-compensacion";
                  break;
                case "ENFERMEDAD":
                  letra = "E";
                  claseExtra += " tipo-enfermedad";
                  break;
                case "MUDANZA":
                  letra = "M";
                  claseExtra += " tipo-mudanza";
                  break;
                case "NACIMIENTO":
                  letra = "N";
                  claseExtra += " tipo-nacimiento";
                  break;
                case "ACCIDENTE":
                  letra = "A";
                  claseExtra += " tipo-accidente";
                  break;
                case "PARO/ASAMBLEA":
                  letra = "PA";
                  claseExtra += " tipo-paro";
                  break;
                case "OTRA":
                  letra = "O";
                  claseExtra += " tipo-otra";
                  break;
                case "COMPENSACIÓN HORAS":
                  letra = "CH";
                  claseExtra += " tipo-compensacion-horas";
                  break;
                case "EXAMEN":
                  letra = "EX";
  ...
  function activarSeleccionFilas() {
    ...
    filas.forEach((fila) => {
      fila.addEventListener("click", () => {
        ...
        document
          .querySelectorAll(".fila-seleccionada")
