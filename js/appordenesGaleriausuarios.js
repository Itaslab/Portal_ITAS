...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  const selectGrupoBKPEditable = document.getElementById(
    "modalGrupoBKPEditable",
  );
  ...
  function renderTabla(lista) {
    ...
    lista.forEach((uRaw) => {
      ...
      const licEnTrue =
        typeof licValor === "string" && licValor.toLowerCase() === "true";
      const activoReal =
        typeof uRaw.activo === "string"
          ? uRaw.activo.trim()
          : uRaw.activo == 1 || uRaw.activo === true
            ? "Activo"
            : "Inactivo";
      ...
      select.addEventListener("change", async (e) => {
        ...
        try {
          const resp = await fetch(
            basePath + `/usuarios/${u.id_usuario}/asignar`,
            {
              method: "POST",
          ...
          const data = await resp.json();
          if (!data.success)
            throw new Error(data.error || "Error al actualizar Asignar");
          await cargarUsuarios();
        } catch (err) {
  ...
  function aplicarFiltros() {
    const grupo = (
      filtroGrupo && filtroGrupo.value ? filtroGrupo.value : ""
    ).toLowerCase();
    const nombre = (
      filtroNombre && filtroNombre.value ? filtroNombre.value : ""
    ).toLowerCase();
    const activoVal = normalizeActivo(filtroActivo ? filtroActivo.value : "");
    const filtrados = usuarios.filter((uRaw) => {
      const nombreRaw = (uRaw.nombre ?? uRaw.Nombre ?? "")
        .toString()
        .toLowerCase();
      const grupoRaw = (uRaw.grupo ?? uRaw.Grupo ?? "")
        .toString()
        .toLowerCase();
      const licValor = uRaw.Lic_Estado ?? uRaw.lic_estado;
      const esLicencia =
        typeof licValor === "string" && licValor.toLowerCase() === "true";
      const activoTexto = esLicencia
        ? "licencia"
        : normalizeActivo(uRaw.activo);
      return (
        (!grupo || grupoRaw === grupo) &&
