...
function mostrarUsuariosEnTabla(usuarios) {
  ...
  if (usuarios.length === 0) {
    tabla.innerHTML =
      '<tr><td colspan="4" class="text-center text-muted">No se encontraron usuarios</td></tr>';
    return;
  }
...
function seleccionarUsuario(fila) {
  ...
  document.getElementById("legajoSeleccionado").textContent =
    usuario.legajo || "-";
...
function configurarEventos() {
  ...
  // Confirmar blanqueo
  document
    .getElementById("btnConfirmarBlanqueo")
...
function mostrarModalConfirmacion() {
  document.getElementById("usuarioConfirm").textContent =
...
async function blanquearPassword() {
  try {
    ...
    // RESTABLECER BOTÓN
    document.getElementById("btnBlanquear").innerHTML =
      '<i class="bi bi-key"></i> Blanquear Contraseña';

    // Mostrar modal de éxito
    document.getElementById("usuarioExito").textContent =
      usuarioSeleccionado.nombre_completo;
