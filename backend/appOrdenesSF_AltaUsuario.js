...
router.get("/usuarios_base", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, mensaje: "Error interno", error: error.message });
  }
});
...
router.post("/usuariosordenes", async (req, res) => {
  ...
  if (typeof MaxPorTrabajar === "undefined" || MaxPorTrabajar === null)
  ...
  if (faltantes.length > 0) {
    return res
      .status(400)
      .json({ mensaje: `Faltan campos: ${faltantes.join(", ")}` });
  }
  ...
  if (String(Modo).toUpperCase() === "SCRIPT") {
    if (!Script || String(Script).trim() === "") {
      return res
        .status(400)
        .json({ mensaje: "Modo SCRIPT requiere campo Script." });
    }
  }
  ...
  if (isNaN(maxInt)) {
    return res
      .status(400)
      .json({ mensaje: "MaxPorTrabajar debe ser un número entero." });
  }
  ...
  if (!horaDeNorm || !horaANorm) {
    return res.status(400).json({
      mensaje: "Formato de HoraDe/HoraA inválido. Use HH:MM o HH:MM:SS.",
    });
  }
  ...
  try {
    ...
    if (hasIdUsuario) {
      ...
      const usuarioBase =
        req.body && req.body.UsuarioBase
          ? String(req.body.UsuarioBase).trim()
          : null;
      if (!usuarioBase) {
        return res.status(400).json({
          mensaje:
            "Se requiere UsuarioBase (legajo) cuando la tabla usa ID_Usuario.",
        });
      }
      ...
      if (!idUsuario) {
        return res.status(400).json({
          mensaje:
            "No se encontró ID_Usuario para el UsuarioBase proporcionado.",
        });
      }
      ...
      const existente = await pool
        ...
        .query(
          `SELECT COUNT(1) AS cnt FROM ${schema}.APP_ORDENES_USR WHERE ID_Usuario = @ID_Usuario`,
        );
      ...
      if (
        ...
        existente.recordset[0].cnt > 0
      ) {
        return res.status(409).json({
          mensaje: "Ya existe un usuario de orden para ese UsuarioBase.",
        });
      }
      ...
      const existente = await pool
        ...
        .query(
          `SELECT COUNT(1) AS cnt FROM ${schema}.APP_ORDENES_USR WHERE Apellido = @Apellido`,
        );
      ...
      if (
        ...
        return res
          .status(409)
          .json({ mensaje: "Ya existe un usuario con ese apellido." });
      }
